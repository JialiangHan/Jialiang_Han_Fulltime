...
public:
  ...
  static Node3D* path_planner(Node3D& start, const Node3D& goal, Node3D* nodes3D,
                              CollisionDetection& configurationSpace, int width, int height);
  // trace path from goal, put all its parents into path
  void trace_path(const Node3D* goal, vector<Node3D> path = vector<Node3D>());
  const vector<Node3D>& get_path()
  {
    return path;
  }
