...
public:
  ...
  float get_cost_so_far() const
  {
    return cost_so_far;
  }
  float get_cost_to_go() const
  {
    return cost_to_go;
  }
  ...
  const Node3D* get_predecessor() const
  {
    return predecessor;
  }
  ...
  void set_theta(const int& theta)
  {
    this->theta = theta;
  }
  void set_cost_so_far(const float& cost_so_far)
  {
    this->cost_so_far = cost_so_far;
  }
  void set_cost_to_go(const float& cost_to_go)
  {
    this->cost_to_go = cost_to_go;
  }
  int set_index(int width)
  {
  ...
  // set predecessor
  void set_predecessor(const Node3D* predecessor)
  {
    this->predecessor = predecessor;
  }

  // udpate cost so far from its predecessor
  void update_cost_so_far()
  {
    cost_so_far = cost_so_far + movement_cost(*predecessor);
  }
  void update_cost_to_go(const Node3D& goal)
  {
    cost_to_go = movement_cost(goal);
  }
  float movement_cost(const Node3D& predecessor) const
  {
    return 10 * sqrt((x - predecessor.x) * (x - predecessor.x) + (y - predecessor.y) * (y - predecessor.y));
  }
